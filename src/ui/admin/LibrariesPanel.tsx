...
export default function LibrariesPanel({ className }: Props) { 
  const libraries = useClientStatesStore(s => s.libraries);
  const updateLibrary = useClientStatesStore(s => s.updateLibrary);
  console.log('[LibrariesPanel] libraries:', libraries.length);
  
  const [scanStatusMap, setScanStatusMap] = useState<Record<string, ScanStatus>>({});
  ...
  const getLibraryScanStatus = (libraryId: string): ScanStatus => {
    return scanStatusMap[libraryId] || 'idle';
  };

  const updateLibraryScanStatus = (libraryId: string, status: ScanStatus) => {
    setScanStatusMap(prev => ({
      ...prev,
      [libraryId]: status
    }));
  };
  ...
  const handleScan = async (libraryId: string) => {
    // 1. 设置扫描状态
    updateLibraryScanStatus(libraryId, 'scanning');
    ...
    try {
      ...
      console.log(`[LibrariesPannel] Start scan task ${task.id} for library ${libraryId}`);
  ...
  const pollTaskStatus = async (libraryId: string, taskId: string) => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        const task = result.data as Task;
        console.log(`[LibrariesPannel] Task ${taskId} status:`, task.status);
        
        // 更新进度信息
        if (task.status === 'completed') {
          // 任务完成, 重置扫描状态
          updateLibraryScanStatus(libraryId, 'idle');
          updateLibrary(libraryId, { lastScan: task.completedAt });
  ...
  const handleLibraryEditSuccess = () => {
    setIsModalOpen(false);
    setEditingLibrary(null);
  ...
  const handleDelete = (id: string) => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        // 在 action 中使用 revalidatePath 强制客户端重新渲染
      } else {
  ...
  return (
    <div className={className}>
      ...
      <ul className="list max-h-[70vh] min-h-[50vh] overflow-y-auto">
        {libraries.map(
          (library) => { 
            const scanStatus = getLibraryScanStatus(library.id);
            ...
            return (
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {scanStatus === 'scanning' ? 'Scaning' : 'Scan'}
                </button>
                {/* 操作菜单 */}
        ...
        <dialog className="modal modal-open">
          <div className="modal-box max-w-4xl">
            ...
            <LibraryEdit 
              ...
              onSuccess={handleLibraryEditSuccess}
